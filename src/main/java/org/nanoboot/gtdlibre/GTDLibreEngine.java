...
import org.nanoboot.gtdlibre.gui.StateMachine;
import org.nanoboot.gtdlibre.journal.JournalModel;
import org.nanoboot.gtdlibre.model.ActionEvent;
...
public class GTDLibreEngine {
    ...
    public GTDLibreEngine() throws FileNotFoundException, XMLStreamException,
            FactoryConfigurationError, MalformedURLException {
    }
        ...
        if (gtdModel == null) {
            ...
            if (getDataFile().exists()) {
                try {
                    ...
                    GTDModel.DataHeader[] dh = findBackupFiles();
                ...
                if (dh != null && dh.length > 0) {
                    handleFailedLoad(gtdModel, dh, 0, new FileNotFoundException(
                            "Missing main data file: '" + getDataFile()
                                    .getAbsolutePath() + "'."));
                }
            }
            setAutoSave(getGlobalProperties()
    ...
    private boolean handleFailedLoad(GTDModel m, GTDModel.DataHeader[] dh,
            int i, Exception e) {
        if (dh == null || dh.length <= i) {
            ...
            int option = JOptionPane.showConfirmDialog(
                    null,
                    "Your data was NOT properly loaded into the GTD-Libre because of file loading error: \n\""
                    +
                    e.toString().replace(". ", ".\n") +
                    ...
                    +
                    "\n\nTip: your data files are stored in the folder: '"
                    + ApplicationHelper.getDataFolder().getAbsolutePath() +
                    "'.\nMain data file '" +
                    getDataFile().getName() +
                    "' has ten backup copies, they are named as" +
                    "\ngtd-libre-data.backupN.xml where 'N' stands for numbers from 0 to 9.",
                    "GTD-Libre - Error Loading Data File",
        ...
        int option = JOptionPane.showConfirmDialog(
                ...
                "',\nsaved on " + ApplicationHelper
                        .toISODateTimeString(dh[i].getModified()) + "?" +
                "\n\n'No' will continue starting the application with partially loaded data."
                +
        ...
        if (option == JOptionPane.YES_OPTION) {
            try {
                System.out.println(
                        "Loading " + dh[i].getFile().getAbsolutePath());
    ...
    private GTDModel.DataHeader[] findBackupFiles() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (f.exists() && f.length() > 0) {
                try {
                    GTDModel.DataHeader dh = new GTDModel.DataHeader(f);
                    l.add(dh);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void save()
        ...
        if (backup.exists() && !backup.delete()) {
            throw new IOException(
                    "Failed to remove backup file '" + backup.getAbsolutePath()
                    + "'.");
        }
        if (getDataFile().exists() && !getDataFile().renameTo(backup)) {
            throw new IOException("Failed to make backup copy file '" + backup
                    .getAbsolutePath() + "'.");
        }
    ...
    public void emergencySave() {
        File save = new File(ApplicationHelper.getDataFolder(),
                ApplicationHelper.SHUTDOWN_EMERGENCY_BACKUP_DATA_FILE_NAME);
        if (save.exists() && !save.delete()) {
            new IOException("Failed to remove emergency backup file '" + save
                    .getAbsolutePath() + "'.").toString();
        }
        try {
            gtdModel.store(save);
            System.out.println("Shutdown emergency backup saved to " + save
                    .getAbsolutePath());
        } catch (Exception e) {
            System.out.println(
                    "Failed to make shutdown emergency backup to " + save
                            .getAbsolutePath());
    ...
    public GlobalProperties getGlobalProperties() {
        if (globalProperties == null) {
            ...
            File f = new File(getDataFolder(),
                    ApplicationHelper.OPTIONS_FILE_NAME);
            if (f.exists()) {
     ...
     */
    public boolean close(final boolean terminal)
            throws IOException, XMLStreamException, FactoryConfigurationError {
