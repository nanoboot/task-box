...
import org.nanoboot.gtdlibre.gui.ExecutePane;
import org.nanoboot.gtdlibre.gui.ImportExampleDialog;
import org.nanoboot.gtdlibre.gui.InBasketPane;
import org.nanoboot.gtdlibre.gui.JournalPane;
import org.nanoboot.gtdlibre.gui.OrganizePane;
import org.nanoboot.gtdlibre.gui.ProcessPane;
import org.nanoboot.gtdlibre.gui.QuickCollectPanel;
import org.nanoboot.gtdlibre.model.ConsistencyException;
...
import org.nanoboot.gtdlibre.model.GTDModelListener;
import org.nanoboot.gtdlibre.model.Utils;
...
public class GTDLibreMain {
    ...
    public static void main(final String[] args) {
        ...
        System.out.println("java -jar gtd-libre.jar [-data <folder>]");
        //                           1         2         3         4         5         6         7         8
        //                  123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890
        System.out.println(
                " * <folder> the folder, where data and configurations is stored,\n         if omited default folder is used in user's home folder.");
        ...
        if (!ApplicationHelper.tryLock(f)) {
            System.out
                    .println("Instance of GTD-Libre already running, exiting.");
            System.exit(0);
        }

        try {
            final GTDLibreMain application = new GTDLibreMain();
    ...
    private JFrame getJFrame() {
        if (jFrame == null) {
            ...
            String windowTitleSuffix =
                    System.getProperty("window.title.suffix", "");
            if (!windowTitleSuffix.isBlank()) {
                windowTitleSuffix = " (" + windowTitleSuffix + ")";
            }
            jFrame.setTitle("GTD-Libre" + windowTitleSuffix);
        ...
        if (actionMap == null) {
            ...
            AbstractAction a = new AbstractAction("Show Closed Actions") {
                ...
                public void actionPerformed(ActionEvent e) {
                    getEngine().getGlobalProperties()
                            .putProperty(GlobalProperties.SHOW_ALL_ACTIONS,
                                    !getEngine().getGlobalProperties()
                                            .getBoolean(
            ...
            a.putValue(Action.SHORT_DESCRIPTION,
                    "Shows closed actions in folders, works only in Organize tab.");
            ...
            a = new AbstractAction("Show Closed Folders & Projects") {
                ...
                public void actionPerformed(ActionEvent e) {
                    getEngine().getGlobalProperties()
                            .putProperty(GlobalProperties.SHOW_CLOSED_FOLDERS,
                                    !getEngine().getGlobalProperties()
                                            .getBoolean(
            ...
            a.putValue(Action.SHORT_DESCRIPTION,
                    "Shows closed folders and projects, works only in Organize tab.");
            ...
            a = new AbstractAction("Show Overview Tab") {
                ...
                public void actionPerformed(ActionEvent e) {
                    getEngine().getGlobalProperties()
                            .putProperty(GlobalProperties.SHOW_OVERVIEW_TAB,
                                    !getEngine().getGlobalProperties()
                                            .getBoolean(
                                                    GlobalProperties.SHOW_OVERVIEW_TAB));
                }
            };
            ...
            a = new AbstractAction("Show Quick Collect Bar") {
                ...
                public void actionPerformed(ActionEvent e) {
                    getEngine().getGlobalProperties()
                            .putProperty(GlobalProperties.SHOW_QUICK_COLLECT,
                                    !getEngine().getGlobalProperties()
                                            .getBoolean(
                                                    GlobalProperties.SHOW_QUICK_COLLECT));
                }
            };
            a.putValue(Action.SHORT_DESCRIPTION,
                    "Shows bar with input for quick adding items to in-basket.");
            ...
            a.putValue(Action.SHORT_DESCRIPTION,
                    "Opens dialog for inporting GTD_Free XML with example lists and some helpful actions..");
    ...
    private JPanel getJContentPane() {
        if (jContentPane == null) {
            ...
            if (getEngine().getGlobalProperties()
                    .getBoolean(GlobalProperties.SHOW_OVERVIEW_TAB, true)) {
                tabbedPane.addTab("Overview", ApplicationHelper
                                .getIcon(ApplicationHelper.icon_name_small_overview),
                        getOverviewPane());
            }

            inBasketPane = new InBasketPane();
            inBasketPane.setEngine(getEngine());
            tabbedPane.addTab("Collect", ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_collecting),
                    inBasketPane);

            processPane = new ProcessPane();
            processPane.setEngine(getEngine());
            tabbedPane.addTab("Process", ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_processing),
                    processPane);

            organizePane = new OrganizePane();
            organizePane.setEngine(getEngine());
            tabbedPane.addTab("Organize/Review", ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_review),
                    organizePane);

            executePane = new ExecutePane();
            executePane.setEngine(getEngine());
            tabbedPane.addTab("Execute (" + getSummaryBean().getQueueCount()
                              + ")", ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_queue_execute),
                    executePane);
            executeTabIndex = tabbedPane.getTabCount() - 1;

            getSummaryBean().addPropertyChangeListener("queueCount",
                    evt -> tabbedPane.setTitleAt(executeTabIndex,
                            "Execute (" + getSummaryBean().getQueueCount()
                            + ")"));

            if (Boolean.parseBoolean(getEngine().getConfiguration()
                    .getProperty("journal.enabled", "false"))) {
                journalPane = new JournalPane();
                journalPane.setEngine(getEngine());
                tabbedPane.addTab("Journal", ApplicationHelper
                                .getIcon(ApplicationHelper.icon_name_large_journaling),
                        journalPane);
            }
...
/*	private void restoreState() {
    ...
    private void enableQuickCollectPanel() {
        quickCollectPanel.setVisible(getEngine().getGlobalProperties()
                                             .getBoolean(
                                                     GlobalProperties.SHOW_QUICK_COLLECT)
    ...
    private Component getOverviewPane() {
        if (overview == null) {
            ...
            l.setFont(l.getFont()
                    .deriveFont((float) (l.getFont().getSize() * 5.0 / 4.0))
                    .deriveFont(Font.BOLD));
            overview.add(l, new GridBagConstraints(0, row++, 1, 1, 1, 0,
                    GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL,
                    new Insets(18, 18, 7, 18), 0, 0));

            overview.add(new OverviewTabPanel(ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_large_collecting),
                            TAB_COLECT, "Collect"),
                    new GridBagConstraints(0, row++, 1, 1, 1, 0,
                            GridBagConstraints.CENTER,
                            GridBagConstraints.HORIZONTAL,
                            new Insets(0, 18, 4, 18), 0, 0));
            overview.add(new OverviewTabPanel(ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_large_processing),
                            TAB_PROCESS, "Process"),
                    new GridBagConstraints(0, row++, 1, 1, 1, 0,
                            GridBagConstraints.CENTER,
                            GridBagConstraints.HORIZONTAL,
                            new Insets(0, 18, 4, 18), 0, 0));
            overview.add(new OverviewTabPanel(ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_large_review),
                            TAB_ORGANIZE, "Organize/Review"),
                    new GridBagConstraints(0, row++, 1, 1, 0, 0,
                            GridBagConstraints.CENTER,
                            GridBagConstraints.HORIZONTAL,
                            new Insets(0, 18, 4, 18), 0, 0));
            overview.add(new OverviewTabPanel(ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_large_queue_execute),
                            TAB_EXECUTE, "Execute"),
                    new GridBagConstraints(0, row++, 1, 1, 1, 0,
                            GridBagConstraints.CENTER,
                            GridBagConstraints.HORIZONTAL,
            ...
            SummaryLabel sl = new SummaryLabel("inbucketCount") {
                ...
                void updateText(PropertyChangeEvent arg0) {
                    if (getSummaryBean().getInbucketCount() > 0) {
                        label.setText(getSummaryBean().getInbucketCount()
                                      + " items in in-bucket list. Process them.");
                        button.setVisible(true);
                    } else {
                        label.setText(getSummaryBean().getInbucketCount()
                                      + " items in in-bucket list.");
                        button.setVisible(false);
                    }
                }
            };
            overview.add(sl, new GridBagConstraints(0, row++, 1, 1, 1, 0,
            ...
            sl = new SummaryLabel("pastActions") {
                ...
                void updateText(PropertyChangeEvent arg0) {
                    if (getSummaryBean().getPastActions() > 0) {
                        label.setText(getSummaryBean().getPastActions()
                                      + " actions are past reminder date. Update them.");
                        button.setVisible(true);
                    } else {
                        label.setText(getSummaryBean().getPastActions()
                                      + " actions are past reminder date.");
                        button.setVisible(false);
            ...
            sl = new SummaryLabel("todayActions") {
                ...
                void updateText(PropertyChangeEvent arg0) {
                    if (getSummaryBean().getTodayActions() > 0) {
                        label.setText(getSummaryBean().getTodayActions()
                                      + " actions are due today. Open tickler.");
                        button.setVisible(true);
                    } else {
                        label.setText(getSummaryBean().getTodayActions()
                                      + " actions are due today.");
                        button.setVisible(false);
            ...
            sl = new SummaryLabel("queueCount") {
                ...
                void updateText(PropertyChangeEvent arg0) {
                    if (getSummaryBean().getQueueCount() > 0) {
                        label.setText(getSummaryBean().getQueueCount()
                                      + " actions queued in \"Next Action\" list. Get things done!");
                        button.setVisible(true);
                    } else {
                        label.setText(getSummaryBean().getQueueCount()
                                      + " actions queued in \"Next Action\" list.");
                        button.setVisible(false);
            ...
            sl = new SummaryLabel("mainCounts") {
                ...
                @Override
                public void actionPerformed(ActionEvent e) {
                    GTDLibreMain.this.getActionMap().get("importDialog")
                            .actionPerformed(e);
                    engine.getGlobalProperties()
                            .putProperty("examplesImported", true);
                    updateText(
                            new PropertyChangeEvent(this, "mainCounts", -1, 1));
                }

                @Override
                void updateText(PropertyChangeEvent arg0) {
                    if (!getEngine().getGlobalProperties()
                            .getBoolean("examplesImported", false)) {
                        label.setText(getSummaryBean().getOpenCount()
                                      + " open items out of total "
                                      + getSummaryBean().getTotalCount()
                                      + " items. From Help menu you can import file with example lists.");
                        button.setVisible(true);
                    } else {
                        label.setText(getSummaryBean().getOpenCount()
                                      + " open items out of total "
    ...
    private JMenu getFileMenu() {
        if (fileMenu == null) {
            ...
            autoSaveMenuItem.setSelected(getEngine().getGlobalProperties()
                    .getBoolean(GlobalProperties.AUTO_SAVE, true));
            ...
            autoSaveMenuItem.addActionListener(e -> {
                ...
                getEngine().getGlobalProperties()
                        .putProperty(GlobalProperties.AUTO_SAVE,
    ...
    private void importFile() {
        if (JFileChooser.APPROVE_OPTION == getFileChooser()
                .showOpenDialog(getJFrame())) {

            try {
                getEngine().getGTDModel()
                        .importFile(getFileChooser().getSelectedFile());
                JOptionPane.showMessageDialog(getJFrame(),
                        "File '" + getFileChooser().getSelectedFile()
                        + "'succesfully imported.", "File import done",
                        JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception e) {
                e.printStackTrace();
                JOptionPane.showMessageDialog(getJFrame(),
                        "Failed to import file '" + getFileChooser()
                                .getSelectedFile() + "'\nfor reason: " + e
                                .getMessage(), "File import error",
    ...
    private void exportFile() {
        if (JFileChooser.APPROVE_OPTION == getFileChooser()
            ...
            try {
                ...
                JOptionPane.showMessageDialog(getJFrame(),
                        "Data succesfully exported to file '" + f + "'.",
                        "File export done", JOptionPane.INFORMATION_MESSAGE);
            } catch (Exception e) {
                e.printStackTrace();
                JOptionPane.showMessageDialog(getJFrame(),
                        "Failed to export to file '" + f + "'\nfor reason: " + e
                                .getMessage(), "File export error",
                        JOptionPane.ERROR_MESSAGE);
            }
        }
    }
    ...
    private JMenu getHelpMenu() {
        if (helpMenu == null) {
            ...
            JMenuItem jmi = new JMenuItem("GTD-Libre Home Page");
            jmi.setIcon(ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_large_browser));
            jmi.addActionListener(e -> {
                URI uri;
                try {
                    uri = new URI(getEngine().getConfiguration()
                            .getProperty("application.home.url",
                                    "http://gtd-libre.sourceforge.net"));
    ...
    private JMenuItem getExitMenuItem() {
        if (exitMenuItem == null) {
            ...
            exitMenuItem.setIcon(ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_large_exit));
    ...
    private JMenuItem getAboutMenuItem() {
        if (aboutMenuItem == null) {
            ...
            aboutMenuItem.setIcon(ApplicationHelper
    ...
    private JDialog getAboutDialog() {
        if (aboutDialog == null) {
            ...
            aboutDialog.setTitle("About GTD-Libre");
            ...
            JLabel jl = new JLabel("GTD-Libre", ii, SwingConstants.CENTER);
            ...
            String s = "Version " + getEngine().getConfiguration()
                    .getProperty("build.version") + " " + getEngine()
                               .getConfiguration().getProperty("build.type");
            jp.add(new JLabel(s, SwingConstants.CENTER),
                    new GridBagConstraints(0, 1, 1, 1, 1, 0,
                            GridBagConstraints.CENTER,
                            GridBagConstraints.HORIZONTAL,
                            new Insets(11, 11, 11, 11), 0, 0));
            jp.add(new JLabel("Copyright © 2008 ikesan@users.sourceforge.net",
                            SwingConstants.CENTER),
                    new GridBagConstraints(0, 2, 1, 1, 1, 0,
                            GridBagConstraints.CENTER,
                            GridBagConstraints.HORIZONTAL,
                            new Insets(11, 11, 11, 11), 0, 0));
            jp.add(new JLabel("Copyright © 2024 mail@robertvokac.com",
                            SwingConstants.CENTER),
            ...
            jp.add(new JScrollPane(jt), new GridBagConstraints(0, 0, 1, 1, 1, 1,
            ...
            jp.add(new JScrollPane(ta), new GridBagConstraints(0, 0, 1, 1, 1, 1,
                    GridBagConstraints.CENTER, GridBagConstraints.BOTH,
                    new Insets(11, 11, 11, 11), 0, 0));
            ...
            ta.setEditable(false);
            jp.add(new JScrollPane(consistencyCheckText),
                    new GridBagConstraints(0, 0, 1, 1, 1, 1,
                            GridBagConstraints.CENTER, GridBagConstraints.BOTH,
                            new Insets(11, 11, 4, 11), 0, 0));
            ...
            b.addActionListener(e -> {
                ...
                consistencyCheckText
                        .setText("Data consistency proved to be OK.");
            });
            jp.add(b, new GridBagConstraints(0, 1, 1, 1, 1, 0,
                    GridBagConstraints.CENTER, GridBagConstraints.NONE,
                    new Insets(4, 11, 11, 11), 0, 0));
    ...
    private JMenuItem getSaveMenuItem() {
        if (saveMenuItem == null) {
            ...
            saveMenuItem.setIcon(ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_large_save));
    ...
    private JFileChooser getFileChooser() {
        if (fileChooser == null) {
            ...
            fileChooser.setFileFilter(
                    new FileNameExtensionFilter("GTD-Libre XML file", "xml"));
        }
        return fileChooser;
    }
    ...
    private JCheckBoxMenuItem getShowAllActionsMenuItem() {
        if (showAllActions == null) {
            showAllActions = new JCheckBoxMenuItem(
                    getActionMap().get(GlobalProperties.SHOW_ALL_ACTIONS));
            showAllActions.setSelected(getEngine().getGlobalProperties()
                    .getBoolean(GlobalProperties.SHOW_ALL_ACTIONS));
            getEngine().getGlobalProperties().addPropertyChangeListener(
                    GlobalProperties.SHOW_ALL_ACTIONS,
                    evt -> showAllActions.setSelected(
                            getEngine().getGlobalProperties().getBoolean(
                                    GlobalProperties.SHOW_ALL_ACTIONS)));
        }
    ...
    private JCheckBoxMenuItem getShowClosedFoldersMenuItem() {
        if (showClosedFolders == null) {
            showClosedFolders = new JCheckBoxMenuItem(
                    getActionMap().get(GlobalProperties.SHOW_CLOSED_FOLDERS));
            showClosedFolders.setSelected(getEngine().getGlobalProperties()
                    .getBoolean(GlobalProperties.SHOW_CLOSED_FOLDERS));
            getEngine().getGlobalProperties().addPropertyChangeListener(
                    GlobalProperties.SHOW_CLOSED_FOLDERS,
                    evt -> showClosedFolders.setSelected(
                            getEngine().getGlobalProperties().getBoolean(
                                    GlobalProperties.SHOW_CLOSED_FOLDERS)));
        }
    ...
    private JCheckBoxMenuItem getShowOverviewTabMenuItem() {
        if (showOverviewTab == null) {
            showOverviewTab = new JCheckBoxMenuItem(
                    getActionMap().get(GlobalProperties.SHOW_OVERVIEW_TAB));
            showOverviewTab.setSelected(getEngine().getGlobalProperties()
                    .getBoolean(GlobalProperties.SHOW_OVERVIEW_TAB, true));
            getEngine().getGlobalProperties().addPropertyChangeListener(
                    GlobalProperties.SHOW_OVERVIEW_TAB,
                    evt -> {
                        showOverviewTab.setSelected(
                                getEngine().getGlobalProperties().getBoolean(
                                        GlobalProperties.SHOW_OVERVIEW_TAB));

                        if (getEngine().getGlobalProperties().getBoolean(
                                GlobalProperties.SHOW_OVERVIEW_TAB)) {
                            if (tabbedPane.getComponentAt(0)
                                != getOverviewPane()) {
                                tabbedPane.insertTab("Overview",
                                        ApplicationHelper.getIcon(
                                                ApplicationHelper.icon_name_small_overview),
                                        getOverviewPane(), "", 0);
                            }
                        } else {
                            if (tabbedPane.getComponentAt(0)
    ...
    private JCheckBoxMenuItem getShowQuickCollectBarMenuItem() {
        if (showQuickCollectBar == null) {
            showQuickCollectBar = new JCheckBoxMenuItem(
                    getActionMap().get(GlobalProperties.SHOW_QUICK_COLLECT));
            showQuickCollectBar.setSelected(getEngine().getGlobalProperties()
                    .getBoolean(GlobalProperties.SHOW_QUICK_COLLECT));
            getEngine().getGlobalProperties().addPropertyChangeListener(
                    GlobalProperties.SHOW_QUICK_COLLECT,
                    evt -> {
                        showQuickCollectBar.setSelected(
                                getEngine().getGlobalProperties().getBoolean(
                                        GlobalProperties.SHOW_QUICK_COLLECT));
                        enableQuickCollectPanel();
                    });
    ...
    class OverviewTabPanel extends JPanel {
        ...
        public OverviewTabPanel(final Icon icon, final int tab,
                final String desc) {
            ...
            add(l, new GridBagConstraints(1, 0, 1, 1, 1, 0,
                    GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL,
    ...
    class SummaryBean implements GTDModelListener {
        ...
        public void elementModified(
                org.nanoboot.gtdlibre.model.ActionEvent a) {
            if (a.getAction().getFolder().isQueue() || a.getAction()
                    .isQueued()) {
                updateQueue();
            } else if (a.getAction().getFolder().isInBucket()) {
                updateInBucket();
            } else if (a.getProperty()
                               .equals(org.nanoboot.gtdlibre.model.Action.REMIND_PROPERTY_NAME)
                       || (a.getAction().getRemind() != null && a.getProperty()
                    .equals(org.nanoboot.gtdlibre.model.Action.RESOLUTION_PROPERTY_NAME))) {
                updateReminders();
            }
        }
        ...
        private void updateInBucket() {
            int n = getEngine().getGTDModel().getInBucketFolder()
                    .getOpenCount();
            if (n != inbucketCount) {
        ...
        private void updateReminders() {
            ...
            for (org.nanoboot.gtdlibre.model.Action a : getEngine()
                    .getGTDModel().getRemindFolder()) {
                if (a.getRemind() != null && a.isOpen()) {
                    long t = a.getRemind().getTime();
                    if (t < time) {
                        past++;
                    } else if ((t >= time) && (t < (time
                                                    + Utils.MILLISECONDS_IN_DAY))) {
                        today++;
                    }
                }
            }
    ...
    abstract class SummaryLabel extends JPanel implements ActionListener {
        ...
        public SummaryLabel(final String prop) {
            ...
            button = new JButton();
            button.setIcon(ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_small_next));
            button.addActionListener(this);
            button.setMargin(new Insets(1, 1, 1, 1));
            add(button, new GridBagConstraints(1, 0, 1, 1, 0, 0,
                    GridBagConstraints.WEST, GridBagConstraints.NONE,
                    new Insets(0, 4, 0, 0), 0, 0));
            add(new JPanel(), new GridBagConstraints(1, 0, 1, 1, 1, 0,
                    GridBagConstraints.WEST, GridBagConstraints.NONE,
                    new Insets(0, 0, 0, 0), 0, 0));
