...
public class ImportExampleDialog {
	...
	class ImportThread extends Thread {
		...
		private void importExample(boolean server) throws IOException, XMLStreamException, FactoryConfigurationError {
			...
			if (server) {
				monitor.setNote("Contacting http://gtd-box.sourceforge.net/...");
	...
	public JDialog getDialog(final Frame owner) {
		if (dialog==null) {
			...
			JLabel l= new JLabel("Import GTD-Box database file with demo list and some helpful actions.");
			...
			l= new JLabel("Download and import latest examples from GTD-Box web site.");
