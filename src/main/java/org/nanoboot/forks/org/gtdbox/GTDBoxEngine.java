...
import org.nanoboot.forks.org.gtdbox.gui.StateMachine;
import org.nanoboot.forks.org.gtdbox.journal.JournalModel;
	...
	private boolean handleFailedLoad(GTDModel m, GTDModel.DataHeader[] dh, int i, Exception e) {
		if (dh==null || dh.length<=i) {
			...
			int option= JOptionPane.showConfirmDialog(
					null, 
					"Your data was NOT properly loaded into the GTD-Box because of file loading error: \n\""+
					e.toString().replace(". ", ".\n")+
					...
					"\ngtd-box-data.backupN.xml where 'N' stands for numbers from 0 to 9." ,
					"GTD-Box - Error Loading Data File", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE);
		...
		int option= JOptionPane.showConfirmDialog(
				...
				"' has ten backup copies, they are named as" +
				"\ngtd-box-data.backupN.xml where 'N' stands for numbers from 0 to 9." ,
				"GTD-Box - Error Loading Data File", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.ERROR_MESSAGE);
		if (option==JOptionPane.YES_OPTION) {
			try {
