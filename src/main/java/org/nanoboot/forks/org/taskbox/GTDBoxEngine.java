...
public class GTDBoxEngine {
	...
	public GTDBoxEngine() throws FileNotFoundException, XMLStreamException, FactoryConfigurationError, MalformedURLException {
	}
	...
	private boolean handleFailedLoad(GTDModel m, GTDModel.DataHeader[] dh, int i, Exception e) {
		if (dh==null || dh.length<=i) {
			...
			int option= JOptionPane.showConfirmDialog(
					...
					"Task-Box - Error Loading Data File", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE);
		...
		int option= JOptionPane.showConfirmDialog(
				...
				"' has ten backup copies, they are named as" +
				"\ntask-box-data.backupN.xml where 'N' stands for numbers from 0 to 9." ,
				"Task-Box - Error Loading Data File", JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.ERROR_MESSAGE);
		if (option==JOptionPane.YES_OPTION) {
			try {
