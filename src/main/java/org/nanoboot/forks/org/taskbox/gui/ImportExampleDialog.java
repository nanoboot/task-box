...
public class ImportExampleDialog {
	...
	class ImportThread extends Thread {
		...
		private void importExample(boolean server) throws IOException, XMLStreamException, FactoryConfigurationError {
			...
			if (server) {
				monitor.setNote("Contacting http://task-box.sourceforge.net/...");
				...
				InputStream is= ApplicationHelper.class.getClassLoader().getResourceAsStream(
						"task-box-example.xml");
				if (is!=null) {
					example= is;
				}
			}
	...
	public JDialog getDialog(final Frame owner) {
		if (dialog==null) {
			...
			JLabel l= new JLabel("Import Task-Box database file with demo list and some helpful actions.");
			...
			l= new JLabel("Download and import latest examples from Task-Box web site.");
