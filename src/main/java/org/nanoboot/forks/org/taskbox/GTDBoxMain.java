	...
	class SummaryBean implements GTDModelListener {
		...
		public void elementModified(
			...
			if (a.getAction().getFolder().isQueue()||a.getAction().isQueued()) {
				...
				updateInBucket();
			} else if (a.getProperty().equals(org.nanoboot.forks.org.taskbox.model.Action.REMIND_PROPERTY_NAME) 
					|| (a.getAction().getRemind()!=null && a.getProperty().equals(org.nanoboot.forks.org.taskbox.model.Action.RESOLUTION_PROPERTY_NAME))) {
				updateReminders();
			}
		}
		...
		private void updateReminders() {
			...
			for (org.nanoboot.forks.org.taskbox.model.Action a : getEngine().getGTDModel().getRemindFolder()) {
...
//				}
		...
		if (!ApplicationHelper.tryLock(f)) {
			System.out.println("Instance of Task-Box already running, exiting.");
			System.exit(0);
		}
...
/*	private void restoreState() {
	...
	private JFileChooser getFileChooser() {
		if (fileChooser==null) {
			...
			fileChooser.setFileFilter(new FileNameExtensionFilter("Task-Box XML file","xml"));
		}
		return fileChooser;
	}
