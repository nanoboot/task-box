...
public class FolderTree extends JTree {
    ...
    private void initialize() {
        ...
        addTreeSelectionListener(e -> {
            ...
            if (e.getNewLeadSelectionPath() != null) {
                Object o = ((DefaultMutableTreeNode) e.getNewLeadSelectionPath()
        ...
        setCellRenderer(new DefaultTreeCellRenderer() {
            ...
            public Component getTreeCellRendererComponent(JTree tree,
                ...
                if (node.getUserObject() instanceof Folder) {
                    ...
                    sb.append(' ');
                    sb.append('(');
                    if (f.getType() == Folder.FolderType.BUILDIN_RESOLVED
            ...
            public Component getTreeCellEditorComponent(JTree tree,
                ...
                if (value instanceof DefaultMutableTreeNode
                    && ((DefaultMutableTreeNode) value)
                            .getUserObject() instanceof Folder) {
                    value = ((Folder) ((DefaultMutableTreeNode) value)
        ...
        setTransferHandler(new ActionTransferHandler() {
            ...
            protected boolean importAction(int id, TransferSupport support) {
                try {
                    ...
                    TreePath tp = getClosestPathForLocation(
            ...
            public boolean canImport(TransferSupport support) {
                ...
                DefaultMutableTreeNode n =
                        (DefaultMutableTreeNode) tp.getLastPathComponent();
                if (n != null && n.getUserObject() instanceof Folder
                    && !((Folder) n.getUserObject()).isMeta()
                    && ((Folder) n.getUserObject()).getType()
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                if (getPathForLocation(e.getPoint().x, e.getPoint().y) != null
                    || e.getPoint().x < 30) {
                    return;
                }
                TreePath t = getClosestPathForLocation(e.getPoint().x,
                        e.getPoint().y);
        ...
        gtdModel.addGTDModelListener(new GTDModelListener() {
            ...
            public void folderModified(FolderEvent f) {
                if (f.getProperty() == "closed") {
                    ...
                    } else if (!f.getFolder().isClosed()
                               && folderToNode(f.getFolder()) == null) {
    ...
    private void rebuildTree() {
        ...
        if (defaultFoldersVisible) {
            ...
            pos++;
            while (root.getChildCount() > pos
                   && root.getChildAt(pos) != projects) {
                root.remove(root.getChildCount() - 1);
                rootChange = true;
            }
            if (root.getChildCount() == pos) {
        ...
        pos++;
        while (root.getChildCount() > pos
               && root.getChildAt(pos) != references) {
            root.remove(root.getChildCount() - 1);
            rootChange = true;
        }
        if (root.getChildCount() == pos) {
    ...
    private DefaultMutableTreeNode addToTree(Folder f, boolean notify) {
        ...
        if (f.getType() == Folder.FolderType.ACTION) {
            ...
            n = someday;
        } else if (defaultFoldersVisible && engine.getStateMachine()
                .getDefaultTreeBranch(f.getType())) {
            meta.add(r = new DefaultMutableTreeNode(f, false));
            n = meta;
        } else if (defaultFoldersVisible
                   && f.getType() == Folder.FolderType.PROJECT) {
    ...
    public void addFolder(String name) {
        ...
        if (getSelectionPath() != null
            && getSelectionPath().getPathCount() > 0) {
    ...
    public boolean isFolderRenamePossible() {
        return getSelectedFolder() != null && (
                getSelectedFolder().isUserFolder() || getSelectedFolder()
                        .isProject());
    }
    ...
    public boolean isFolderAddPossible() {
        if (getSelectionPath() != null
            && getSelectionPath().getPathCount() > 0) {
			return getSelectionPath().getPathComponent(1) == someday
				   || getSelectionPath().getPathComponent(1) == references
				   || getSelectionPath().getPathComponent(1) == actions
				   || getSelectionPath().getPathComponent(1) == projects;
        }
        return false;
    }
    ...
    public int[] getExpendedNodes() {
        ...
        Enumeration<TreePath> en =
                getExpandedDescendants(new TreePath(root.getPath()));
    ...
    static class SortedTreeNode extends DefaultMutableTreeNode
        ...
        public int compare(TreeNode o1, TreeNode o2) {
            ...
            if (f1 instanceof Folder && f2 instanceof Folder) {
                return ((Folder) f1).getName()
                        .compareTo(((Folder) f2).getName());
            }
            return 0;
        }
