...
public class ActionTable extends JTable {
    ...
    private void rebuildColumns() {
        ...
        i++;
        if (showFolderColumn) {
            while (cm.getColumnCount() > i
                   && cm.getColumn(i) != getFolderColumn()) {
        ...
        if (showProjectColumn) {
            while (cm.getColumnCount() > i
                   && cm.getColumn(i) != getProjectColumn()) {
                cm.removeColumn(cm.getColumn(i));
            }
            if (cm.getColumnCount() == i) {
        ...
        while (cm.getColumnCount() > i
               && cm.getColumn(i) != getDescriptionColumn()) {
            cm.removeColumn(cm.getColumn(i));
        }
        if (cm.getColumnCount() == i) {
        ...
        if (showPriorityColumn) {
            while (cm.getColumnCount() > i
                   && cm.getColumn(i) != getPriorityColumn()) {
                cm.removeColumn(cm.getColumn(i));
            }
            if (cm.getColumnCount() == i) {
        ...
        if (showRemindColumn) {
            while (cm.getColumnCount() > i
                   && cm.getColumn(i) != getRemindColumn()) {
                cm.removeColumn(cm.getColumn(i));
            }
            if (cm.getColumnCount() == i) {
        ...
        if (showResolvedColumn) {
            while (cm.getColumnCount() > i
                   && cm.getColumn(i) != getResolvedColumn()) {
                cm.removeColumn(cm.getColumn(i));
            }
            if (cm.getColumnCount() == i) {
        ...
        while (cm.getColumnCount() > i
               && cm.getColumn(i) != getActionColumn()) {
            cm.removeColumn(cm.getColumn(i));
        }
        if (cm.getColumnCount() == i) {
    ...
    private TableColumn getRemindColumn() {
        if (remindColumn == null) {
            ...
            remindColumn.setHeaderValue(
                    Messages.getString("action.Reminder")); //$NON-NLS-1$
                ...
                public Component getTableCellRendererComponent(JTable table,
                    ...
                    if (value instanceof Date) {
                        ...
                        if (d.getTime() < today) {
                            ...
                            if (d.getTime()
                                < today + Utils.MILLISECONDS_IN_DAY) {
                                setForeground(Color.RED);
                            } else {
                                setForeground(ActionTable.this.getForeground());
                            }
                            if (d.getTime() - today
                                < 3 * Utils.MILLISECONDS_IN_DAY) {
    ...
    private TableColumn getIDColumn() {
        if (idColumn == null) {
            ...
            idColumn.setHeaderValue(
                    Messages.getString("ActionTable.ID")); //$NON-NLS-1$
        }
    ...
    private TableColumn getDescriptionColumn() {
        if (descriptionColumn == null) {
            ...
            descriptionColumn.setHeaderValue(Messages.getString(
                    "ActionTable.Description")); //$NON-NLS-1$
        }
    ...
    private TableColumn getProjectColumn() {
        if (projectColumn == null) {
            ...
            projectColumn.setHeaderValue(
                    Messages.getString("ActionTable.Project")); //$NON-NLS-1$
    ...
    private TableColumn getFolderColumn() {
        if (folderColumn == null) {
            ...
            folderColumn.setHeaderValue(
                    Messages.getString("ActionTable.Folder")); //$NON-NLS-1$
    ...
    private void initialize() {
        ...
        set.add(AWTKeyStroke
                .getAWTKeyStroke(KeyEvent.VK_TAB, InputEvent.CTRL_MASK));
        ...
        set.add(AWTKeyStroke.getAWTKeyStroke(KeyEvent.VK_TAB,
        ...
        getSelectionModel().addListSelectionListener(e -> {
            if (!e.getValueIsAdjusting()) {
                ...
                enableMoveActions();
                try {
                    firePropertyChange(SELECTED_ACTION_PROPERTY_NAME, old,
                            selectedAction);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
    ...
    public void setFolder(Folder folder, boolean showAll) {
        ...
        if (folder != null) {
            ...
            setShowAll(showAll);
            setShowResolvedColumn(engine.getStateMachine()
                    .getShowResolvedColumn(folder.getType()));
            setShowRemindColumn(engine.getStateMachine()
                    .getShowRemindColumn(folder.getType()));
            setShowPriorityColumn(engine.getStateMachine()
                    .getShowPriorityColumn(folder.getType()));
            setShowProjectColumn(engine.getStateMachine()
                    .getShowProjectColumn(folder.getType()));
            setShowFolderColumn(engine.getStateMachine()
                    .getShowFolderColumn(folder.getType()));
    ...
    private void updateIDColumnWidth() {
        ...
        Component c = getColumnModel().getColumn(0).getCellRenderer()
                .getTableCellRendererComponent(this, id, false, false, 0, 0);
    ...
    private void enableMoveActions() {
        if (moveUpAction != null) {
            moveUpAction.setEnabled(
                    moveEnabled && getSelectedAction() != null && !rowSorter
                            .isTransformed() && engine.getStateMachine()
                            .getChangeActionOrder(getFolder().getType())
                    && getFolder().canMoveUp(getSelectedAction()));
        }
        if (moveDownAction != null) {
            moveDownAction.setEnabled(
                    moveEnabled && getSelectedAction() != null && !rowSorter
                            .isTransformed() && engine.getStateMachine()
                            .getChangeActionOrder(getFolder().getType())
                    && getFolder().canMoveDown(getSelectedAction()));
        }
    }
    ...
    public javax.swing.Action getMoveUpAction() {
        if (moveUpAction == null) {
            moveUpAction = new AbstractAction(
                    Messages.getString("ActionTable.MoveUp")) { //$NON-NLS-1$
                private static final long serialVersionUID =
                        9040402331054547898L;
            ...
            moveUpAction.putValue(javax.swing.Action.SHORT_DESCRIPTION,
                    "Move current action one place up.");
            moveUpAction.putValue(javax.swing.Action.LARGE_ICON_KEY,
                    ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_up));
            moveUpAction.putValue(javax.swing.Action.SMALL_ICON,
                    ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_up));
            moveUpAction.putValue(javax.swing.Action.ACCELERATOR_KEY, KeyStroke
    ...
    public javax.swing.Action getSelectNextAction() {
        if (goNextAction == null) {
            ...
            goNextAction.putValue(javax.swing.Action.SHORT_DESCRIPTION,
                    "Select next action in list.");
            goNextAction.putValue(javax.swing.Action.LARGE_ICON_KEY,
                    ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_next));
            goNextAction.putValue(javax.swing.Action.SMALL_ICON,
                    ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_next));
    ...
    public javax.swing.Action getSelectPreviousAction() {
        if (goPreviousAction == null) {
            ...
            goPreviousAction.putValue(javax.swing.Action.SHORT_DESCRIPTION,
                    "Select previous action in list.");
            goPreviousAction.putValue(javax.swing.Action.LARGE_ICON_KEY,
                    ApplicationHelper.getIcon(
                            ApplicationHelper.icon_name_small_previous));
            goPreviousAction.putValue(javax.swing.Action.SMALL_ICON,
                    ApplicationHelper.getIcon(
                            ApplicationHelper.icon_name_small_previous));
    ...
    public javax.swing.Action getMoveDownAction() {
        if (moveDownAction == null) {
            moveDownAction = new AbstractAction(
                    Messages.getString("ActionTable.MoveDown")) { //$NON-NLS-1$
                private static final long serialVersionUID =
                        4715532030674163250L;
            ...
            moveDownAction.putValue(javax.swing.Action.SHORT_DESCRIPTION,
                    "Move current action one place down.");
            moveDownAction.putValue(javax.swing.Action.LARGE_ICON_KEY,
                    ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_down));
            moveDownAction.putValue(javax.swing.Action.SMALL_ICON,
                    ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_down));
            moveDownAction.putValue(javax.swing.Action.ACCELERATOR_KEY,
    ...
    static class DescriptionCellEditor extends DefaultCellEditor
    ...
    static class PriorityCellEditor extends AbstractCellEditor
            implements TableCellEditor {
        private static final long serialVersionUID = 1L;
    ...
    class ProjectCellEditor extends AbstractCellEditor
            implements TableCellEditor {
        private static final long serialVersionUID = 1L;
        ...
        public Object getCellEditorValue() {
            return combo.getSelectedProject() != null ?
                    combo.getSelectedProject().getId() : null;
        }
    ...
    public class ActionTableModel extends AbstractTableModel
        ...
        public void reload(Folder f, ActionFilter a) {
            ...
            if (f != null) {
                ...
                if (a != null) {
                    for (Action note : f) {
                        if ((showAll || note.isOpen()) && a
                                .isAcceptable(f, note)) {
                            data.add(note);
                            if (showFolderColumn) {
                                int i = getFontMetrics(getFont()).stringWidth(
        ...
        public boolean isCellEditable(int rowIndex, int columnIndex) {
            if (columnIndex == 1) {
				...
				return columnIndex == 4 || columnIndex == 5 || columnIndex == 7
					   || columnIndex == 9;
		}
    ...
    class CellActionEditor extends AbstractCellEditor
            ...
            if (isOpen) {
                ...
                if (cellAction == CellAction.DELETE) {
                    ...
                    button.setIcon(ApplicationHelper
                            .getIcon(ApplicationHelper.icon_name_small_delete));
                    return true;
                }
                if (cellAction == CellAction.RESOLVE) {
                    ...
                    button.setToolTipText(
                            Messages.getString("action.Resolve")); //$NON-NLS-1$
                    button.setIcon(ApplicationHelper.getIcon(
                            ApplicationHelper.icon_name_small_resolve));
                    return true;
                }
                a = CellAction.REOPEN;
                ...
                button.setIcon(ApplicationHelper
                        .getIcon(ApplicationHelper.icon_name_small_undelete));
                return false;
            }
    ...
    class QueueCellEditor extends AbstractCellEditor
            implements TableCellEditor, TableCellRenderer {
        private static final long serialVersionUID = 1L;
        ...
        public QueueCellEditor() {
            ...
            button.setIcon(ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_small_queue_off));
            button.setSelectedIcon(ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_small_queue_on));
            button.setDisabledSelectedIcon(ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_small_queue_off));
            button.setBackground(getBackground());
            button.addActionListener(e -> {
                stopCellEditing();
