...
public class ActionSpinner extends JPanel implements FolderListener {
    ...
    private void initialize() {
        ...
        idLabel = new JLabel(
                Messages.getString("ActionPanel.ID-NA")); //$NON-NLS-1$
        Dimension d = new Dimension(75, 21);
        idLabel.setPreferredSize(d);
        idLabel.setMinimumSize(d);
        jp.add(idLabel, new GridBagConstraints(0, 0, 1, 1, 0, 0,
        ...
        jp.add(sp, new GridBagConstraints(1, 0, 1, 1, 1, 1,
    ...
    private void updateGUI() {
        ...
        if (a == null) {
            idLabel.setText(Messages.getString("ActionPanel.ID") + Messages
                    .getString("ActionPanel.NA")); //$NON-NLS-1$
            descText.setEnabled(false);
            descText.setEditable(false);
            descText.setText(ApplicationHelper.EMPTY_STRING);
        } else {
            setting = true;
            String s = Messages.getString("ActionPanel.ID") + a
                    .getId(); //$NON-NLS-1$
            idLabel.setText(s);
            descText.setEnabled(true);
            descText.setEditable(true);
            s = a.getDescription() == null ? ApplicationHelper.EMPTY_STRING :
                    a.getDescription();
