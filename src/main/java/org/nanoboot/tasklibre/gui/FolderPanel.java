...
import org.nanoboot.tasklibre.model.GTDModelAdapter;
...
public class FolderPanel extends JPanel {
    ...
    private void initialize() {
        ...
        folderTree.addPropertyChangeListener("selectedFolder", evt -> {
            ...
            folderNameField.setEnabled(
                    folderTree.isFolderRenamePossible() || folderTree
                            .isFolderAddPossible());

            getRenameFolderAction()
                    .setEnabled(folderTree.isFolderRenamePossible());
            getAddFolderAction().setEnabled(folderTree.isFolderAddPossible());
            getCloseFolderAction().setEnabled(
                    getSelectedFolder() != null && !getSelectedFolder()
                            .isBuildIn() && getSelectedFolder().getType()
                                            != Folder.FolderType.INBUCKET
                    && getSelectedFolder().getType() != Folder.FolderType.QUEUE
                    && !getSelectedFolder().isClosed()
                    && getSelectedFolder().getOpenCount() == 0);
            getReopenFolderAction().setEnabled(
                    getSelectedFolder() != null && getSelectedFolder()
                            .isClosed());
        ...
        add(new JScrollPane(folderTree),
        ...
        p.add(folderNameField, new GridBagConstraints(0, 0, 2, 1, 1, 0,
                GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL,
                new Insets(0, 0, 0, 0), 0, 0));

        JButton b = new JButton(getRenameFolderAction());
        b.setMargin(new Insets(1, 1, 1, 1));
        p.add(b, new GridBagConstraints(0, 1, 1, 1, 1, 0,
                GridBagConstraints.CENTER, GridBagConstraints.NONE,
                new Insets(4, 0, 4, 0), 0, 0));

        b = new JButton(getAddFolderAction());
        b.setMargin(new Insets(1, 1, 1, 1));
        p.add(b, new GridBagConstraints(1, 1, 1, 1, 1, 0,
                GridBagConstraints.CENTER, GridBagConstraints.NONE,
                new Insets(4, 0, 4, 0), 0, 0));

        b = new JButton(getCloseFolderAction());
        b.setMargin(new Insets(1, 1, 1, 1));
        p.add(b, new GridBagConstraints(0, 2, 1, 1, 1, 0,
                GridBagConstraints.CENTER, GridBagConstraints.NONE,
                new Insets(0, 0, 4, 0), 0, 0));

        b = new JButton(getReopenFolderAction());
        b.setMargin(new Insets(1, 1, 1, 1));
        p.add(b, new GridBagConstraints(1, 2, 1, 1, 1, 0,
                GridBagConstraints.CENTER, GridBagConstraints.NONE,
                new Insets(0, 0, 4, 0), 0, 0));

        foldingPanel = new FoldingPanel();
        foldingPanel.addFold("Edit", p, true, false);
        add(foldingPanel, new GridBagConstraints(0, 1, 1, 1, 1, 0,
                GridBagConstraints.CENTER, GridBagConstraints.BOTH,
                new Insets(0, 0, 0, 0), 0, 0));
    }
    ...
    private Action getAddFolderAction() {
        if (addFolderAction == null) {
            addFolderAction = new AbstractAction("Add", ApplicationHelper
    ...
    private Action getRenameFolderAction() {
        if (renameFolderAction == null) {
            renameFolderAction = new AbstractAction("Rename", ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_large_rename)) {
                private static final long serialVersionUID = 0L;
    ...
    private Action getCloseFolderAction() {
        if (closeFolderAction == null) {
            closeFolderAction = new AbstractAction("Close", ApplicationHelper
            ...
            closeFolderAction.putValue(Action.SHORT_DESCRIPTION,
                    "Closes list or project, which contains no open actions.");
        }
    ...
    private Action getReopenFolderAction() {
        if (reopenFolderAction == null) {
            reopenFolderAction = new AbstractAction("Reopen", ApplicationHelper
                    .getIcon(ApplicationHelper.icon_name_large_undelete)) {
                private static final long serialVersionUID = 0L;
            ...
            reopenFolderAction
                    .putValue(Action.SHORT_DESCRIPTION, "Opens closed list.");
        }
    ...
    public void setEngine(TaskLibreEngine engine) {
        ...
        engine.getGTDModel().addGTDModelListener(new GTDModelAdapter() {
            ...
            public void elementModified(
                ...
                if (a.getAction().getFolder() == getSelectedFolder() || (
                        getSelectedFolder() != null
                        && a.getAction().getProject() != null
                        && getSelectedFolder().getId() == a.getAction()
                                .getProject())) {
                    getCloseFolderAction().setEnabled(
                            getSelectedFolder() != null && !getSelectedFolder()
                                    .isBuildIn()
                            && getSelectedFolder().getType()
                               != Folder.FolderType.INBUCKET
                            && getSelectedFolder().getType()
                               != Folder.FolderType.QUEUE
                            && !getSelectedFolder().isClosed()
                            && getSelectedFolder().getOpenCount() == 0);
                    getReopenFolderAction().setEnabled(
                            getSelectedFolder() != null && getSelectedFolder()
